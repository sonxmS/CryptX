...
export function MainCard({
  ...
  const decryptVal = () => {
    try {
      ...
      setAlertText(
        "Decryption failed: Possibly corrupted input for decryption"
      );
      setShowAlert(true);
      return null;
    }
  ...
  const removePrefix = (input, prefix) => {
    if (input.startsWith(prefix)) {
      ...
      setAlertText(
        `Decryption failed: Prefix "${prefix}" not found at the beginning of "${input}"`
      );
      setShowAlert(true);
      return null;
    }
  ...
  const removeSuffix = (input, suffix) => {
    if (input.endsWith(suffix)) {
      ...
      setAlertText(
        `Decryption failed: Suffix "${suffix}" not found at the end of "${input}"`
      );
      setShowAlert(true);
      return null;
    }
  ...
  const aesDecryption = (encryptedText, key) => {
    if (!encryptedText || !key) {
      setAlertText(
        "Decryption failed: Encrypted text and key are required for AES decryption."
      );
      setShowAlert(true);
      return null;
    }
  ...
  >
    {/* Label */}
    <label htmlFor={inputId} className="text-slate-50 font-semibold text-xl md:text-2xl">
      {label}
    </label>
    ...
    <TextareaComponent
      ...
      ref={input}
      onChange={(e) => throttledSetInput(e.target.value)}
      title={
        !isConfirmed
          ? "Please enter transformation options and press the ✓ (check) button to enable this field"
          : ""
      }
      id={inputId}
      placeholder={placeholderLabel}
      className="text-slate-50 text-sm transition-[border] duration-200 ease-in-out 
                 min-h-60 border-white/10 rounded-md"
    />
